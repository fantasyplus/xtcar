    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  pnh_.param("vehicle_model_wheelbase", wheelbase_, double(2.8));

  //车辆模型选择：共有三种车辆模型可选，dynamics、kinematics和kinematics_no_delay
  /* vehicle model setup */
  ...
  //选择QP优化方法，包括三种方法：unconstraint、unconstraint_fast和qpoases_hotstart
  /* QP solver setup */
  ...
  // twist_raw：发布目标速度指令（目标纵向速度，目标横摆角速度）
  pub_twist_cmd_ = nh_.advertise<geometry_msgs::TwistStamped>(out_twist, 1);

  // ctrl_cmd：发布目标控制指令（目标转向角，目标纵向速度，目标加速度）,对接Can
  pub_steer_vel_ctrl_cmd_ = nh_.advertise<mpc_msgs::ControlCommand>(out_vehicle_cmd, 1);
  // mpc_waypoints:接收mpc_waypoints_converter节点发布的局部路径信息
  sub_ref_path_ = nh_.subscribe(in_waypoints, 1, &MPCFollower::callbackRefPath, this);
  // current_pose：接收车辆当前位姿信息（pose）
  sub_pose_ = nh_.subscribe(in_selfpose, 1, &MPCFollower::callbackPose, this);
  // vehicle_status：接收车辆状态信息（当前车速，当前转向角）
...
void MPCFollower::timerCallback(const ros::TimerEvent &te)
{
  //车辆制动
  /* guard */
  if (vehicle_model_ptr_ == nullptr || qpsolver_ptr_ == nullptr) //模型和优化方法为空
  {
  ...
  if (ref_traj_.size() == 0 || !my_position_ok_ || !my_velocity_ok_ || !my_steering_ok_) //路径信息为空，接收不到车辆的位姿信息
  {
  ...
  //控制命令初始化
  /* control command */
  double vel_cmd = 0.0;       //速度
  double acc_cmd = 0.0;       //加速度
  double steer_cmd = 0.0;     //转向
  double steer_vel_cmd = 0.0; //转向速度
...
bool MPCFollower::calculateMPC(double &vel_cmd, double &acc_cmd, double &steer_cmd, double &steer_vel_cmd)
{
  ...
  for (unsigned int i = 0; i < input_buffer_.size(); ++i)
  {
    ...
    x_curr = Ad * x_curr + Bd * ud + Wd; //预测方程
  ...
  /////////////// generate mpc matrix  ///////////////
  /*
   * predict equation: Xec = Aex * x0 + Bex * Uex + Wex  //预测方程
   * cost function: J = Xex' * Qex * Xex + (Uex - Uref)' * Rex * (Uex - Urefex) //代价函数
   * Qex = diag([Q,Q,...]), Rex = diag([R,R,...]) //Q阵和R阵
   */
  ...
  for (int i = 0; i < N; ++i) //求出每个预测时刻下的参数矩阵
  {
    ...
    //获取参考输入
    //根据每个预测点的曲率求前馈转向角，并且为了减少曲率噪声，当前馈角度小于2度时为0
  ...
  /////////////// optimization ///////////////
  /*
   * solve quadratic optimization. 二次优化
  ...
  // predict equation: Xec = Aex * x0 + Bex * Uex + Wex  //预测方程
  // cost function: J = Xex' * Qex * Xex + (Uex - Uref)' * Rex * (Uex - Urefex) //代价函数
  /* constraint matrix : lb < U < ub, lbA < A*U < ubA  //约束矩阵*/
...
// mpc_follower节点输入一mpc_waypoints:接收mpc_waypoints_converter节点发布的局部路径信息
void MPCFollower::callbackRefPath(const mpc_msgs::Lane::ConstPtr &msg)
{
...
// mpc_follower节点输入二current_pose：接收车辆当前位姿信息
...
// mpc_follower节点输入三vehicle_status：接收车辆状态信息
void MPCFollower::callbackVehicleStatus(const mpc_msgs::VehicleStatus &msg)
{
  //天美SUV，转向角：方向盘转角=1：14.6
  vehicle_status_.tire_angle_rad = msg.steer / 14.6 / 180.0 * M_PI; //当前转向角
...
//通过前面改变output_interface_的值控制节点输出方式
//天美suv：ctrl_raw
...
// mpc_follower节点输出一twist_raw：发布目标速度指令
...
// mpc_follower节点输出二ctrl_cmd：发布目标控制指令
void MPCFollower::publishCtrlCmd(const double &vel_cmd, const double &acc_cmd, const double &steer_cmd)
{
  mpc_msgs::ControlCommand cmd;
  // cmd.header.frame_id = "/base_link";
  // cmd.header.stamp = ros::Time::now();
  cmd.linear_velocity = vel_cmd;     //目标纵向速度
  cmd.linear_acceleration = acc_cmd; //目标加速度
  cmd.steering_angle = steer_cmd;    //目标转向角
