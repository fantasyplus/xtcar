...
public:
  ...
  {
    //发布局部路径信息（只包含最近路径点前后的若干几个点）
    pub_waypoints_ = nh_.advertise<mpc_msgs::Lane>("/mpc_waypoints", 1);
    //接受最近路径点的序号
...
private:
  ...
  void callbackBaseWaypoints(const mpc_msgs::Lane &msg)
  {
    base_waypoints_ = msg;
  }

  void callbackFinalWaypoints(const mpc_msgs::Lane &final_waypoints)
  {
    ...
    //将最近点之前的back_waypoints_num_个路径点（已经跑过的路径点）注入mpc_waypoints
