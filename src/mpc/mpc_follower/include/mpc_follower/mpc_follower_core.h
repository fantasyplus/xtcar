...
private:
  ...
  mpc_msgs::Lane current_waypoints_;                    //!< @brief current waypoints to be followed
   ...
   */
  void callbackRefPath(const mpc_msgs::Lane::ConstPtr &);
  ...
  /**
   * @brief publish control command as mpc_msgs/ControlCommand type
   * @param [in] vel_cmd velocity command [m/s] for vehicle control
   * @param [in] acc_cmd acceleration command [m/s2] for vehicle control
   * @param [in] steer_cmd steering angle command [rad] for vehicle control
   */
